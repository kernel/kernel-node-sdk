...
export async function browserFetch(client, sessionId, input, init) {
    const route = client.browserRouteCache.get(sessionId);
    if (!route) {
        throw new KernelError(`browser route cache does not contain session ${sessionId}; create, retrieve, or list the browser before calling browser.fetch`);
    }
...
//# sourceMappingURL=browser-fetch.mjs.map