...
exports.BrowserRouteCache = void 0;
exports.browserRoutingSubresourcesFromEnv = browserRoutingSubresourcesFromEnv;
exports.createRoutingFetch = createRoutingFetch;
exports.matchesDirectVMPrefix = matchesDirectVMPrefix;
...
exports.BrowserRouteCache = BrowserRouteCache;
...
function browserRoutingSubresourcesFromEnv() {
...
function createRoutingFetch(innerFetch, { apiBaseURL, subresources, cache, }) {
...
function matchesDirectVMPrefix(tail, prefixes) {
...
async function routeRequest(innerFetch, { input, init, request, }, apiOrigin, allowed, cache) {
    ...
    const target = new URL((0, join_url_1.joinURL)(route.baseURL, `/${subresource}${match[3] ?? ''}`));
    ...
    if ((routed.status === 401 || routed.status === 403) && target.searchParams.get('jwt')) {
        ...
        await (0, shims_1.CancelReadableStream)(routed.body);
        return innerFetch(input, init);
    }
    return routed;
}
