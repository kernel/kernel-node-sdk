...
const timeoutError = (id, polls, lastStatus, startedAt) => new APIConnectionTimeoutError({
...
const analysisFinished = (response, requestedID) => {
    ...
    if (!analysis || typeof analysis !== 'object') {
        throw new KernelError(`Config registry response for ${JSON.stringify(requestedID)} is missing an analysis`);
    }
    if (typeof analysis.id !== 'string' || !analysis.id) {
        throw new KernelError(`Config registry response for ${JSON.stringify(requestedID)} has no valid analysis ID`);
    }
    if (analysis.id !== requestedID) {
        throw new KernelError(`Config registry response for ${JSON.stringify(requestedID)} returned analysis ${JSON.stringify(analysis.id)}`);
    }
    ...
    if (analysis.finished_at !== null &&
        ...
        throw new KernelError(`Config registry analysis ${JSON.stringify(requestedID)} has an invalid finished_at`);
    }
    return [analysis.finished_at !== null || terminalStatuses.has(analysis.status), analysis.status];
};
export async function waitForConfigRegistryAnalysis(resource, id, options = {}) {
    if (!id) {
...
//# sourceMappingURL=config-registry-wait.mjs.map